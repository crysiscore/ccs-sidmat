...
export async function createGuia(guiaInfo) {
    try{
        const response = await api.post('/rpc/sp_create_guia_saida', guiaInfo, {headers: {'Prefer': 'return=representation'}});
...
export async function listRequisicoesByGuia(nr_guia) {
    try{
        const response = await api.get('/view_get_requisicoes_by_nr_guia?nr_guia=eq.' + nr_guia);
        return response.data;
    }catch(error) {
        throw error;
