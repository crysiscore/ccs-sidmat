...
export default function RequisicaoPorUS() {
  ...
  const removeSelectedMaterial = (objectToRemove) => {
    const updatedArray = materialRequisicao.filter(
  ...
  const handleEnviarRequisicao = async (rows) => {
    ...
    let requisicoesArray = materialRequisicao.map(
      ({ descricao, ...item }) => item
    );
    // rename the idUS property to unidade_sanitaria
    requisicoesArray = requisicoesArray.map(({ idUS, ...item }) => ({
      ...item,
      unidade_sanitaria: idUS,
    }));
  ...
  if (!locations || !materialList || !pontosFocais) {
    ...
    NotificationManager.info(
      "Nao ha material disponivel para a sua area.",
      "Info",
      8000
    );
    return (
          ...
          <div className="px-2 mx-auto mainCard">
            ...
            <div>
              <Autocomplete
                ...
                sx={{ width: 300 }}
                renderInput={(params) => (
                  <TextField {...params} label="Selecione uma US:" />
                )}
              />
            </div>

            <div className=" gap-y-4 overflow-hidden overflow-y-auto center wrapper-requisicoes ">
              <MaterialDisponivelDistribuicaoUSTable
                colunas={columnNames}
                dados={[]}
