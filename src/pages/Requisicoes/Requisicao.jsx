...
import {
  getAllUnidadesSanitarias,
  getPontosFocais,
} from "../../middleware/GenericService";
...
function NovaRequisicao() {
  ...
  const kamavota = (
    <Box sx={{ display: "flex", flexDirection: "column", fontSize: 8, ml: 2 }}>
      <FormControlLabel
        label="1 de Junho PS"
        value="1"
    ...
    getPontosFocais(userArea)
      ...
      .catch((error) => {
        ...
        NotificationManager.error(
          "Nao foi possivel carregar os pontos focais: " + error.message,
          "Erro: " + error.code,
          8000
        );
      });
  ...
  const removeRequisicao = (idToRemove) => {
    ...
    let quantidadeToRemove = requisicoes.find(
      (item) => item.us === idToRemove
    ).quantidade;
    // if quanityToRemove is empty, then set it to zero
    if (quantidadeToRemove === "") {
      quantidadeToRemove = 0;
    }
    setTotalQuantidadeReq(
      (prevState) => prevState - parseInt(quantidadeToRemove)
    );
    const updatedRequisicoesArray = requisicoes.filter(
      (obj) => obj.us !== idToRemove
    );
  ...
  const handleEnviarRequisicao = async () => {
    ...
    if (requisicoesWithEmptyQuantidade.length > 0) {
      NotificationManager.error(
        "Houve erro: Todas  as requisicoes devem ter  quantidade superior a zero. corrija os dados",
        "Erro",
        8000
      );
      return;
    ...
    requisicoesArray = requisicoesArray.map(({ us, ...item }) => ({
      ...item,
      unidade_sanitaria: us,
    }));
    // change quantidade type to integer
    requisicoesArray = requisicoesArray.map(({ quantidade, ...item }) => ({
      ...item,
  ...
  const handleSetSelected = (usID) => {
    ...
    if (requisicoes.length === 0) {
      ...
      let filteredPontosFocais = pontosFocais.filter(
        (pf) => pf.unidade_sanitaria_id === parseInt(usID)
      );
      ...
      if (tempSeedArray.length > counterRequisicoes) {
        ...
        const newObj = {
          ...
          quantidade: "",
          pf_nome: prefferedPontoFocal ? prefferedPontoFocal[0].nome : "",
          pf_contacto: prefferedPontoFocal
            ? prefferedPontoFocal[0].contacto
            : "",
          material: id_material,
          requisitante: userInfo[0].id,
          notas: "",
        ...
        if (tempSeedArray.length === 0) {
          ...
          let idLastValue = idArray.find((item) => {
            return !tempSeedArray.map(Number).includes(item);
          });
  ...
  const handleSaveRow = ({ exitEditingMode, row, values }) => {
    ...
    if (quantidade === "" || isNaN(quantidade) || parseInt(quantidade) === 0) {
      ...
      NotificationManager.error(
        "Houve erro: A quantidade deve ser um numero e maior que zero",
        "Erro",
        8000
      );
    ...
    if (
      originalRequisicao.quantidade !== "" &&
      valuesCached.quantidade !== originalRequisicao.quantidade
    ) {
      totalQuantidadeReqValue =
        totalQuantidadeReqRef.current -
        parseInt(originalRequisicao.quantidade) +
        parseInt(quantidade);
      // subtract the previous quantidade from totalQuantidadeReq state
      if (totalQuantidadeReqValue > materialRequisicao[0].qtd_stock) {
        NotificationManager.error(
          "Houve erro: As quantidades requisitadas sao superiores ao stock",
          "Erro",
          8000
        );
        exitEditingMode(); //required to exit editing mode
      ...
      if (
        parseInt(quantidade) + totalQuantidadeReqValue >
        materialRequisicao[0].qtd_stock
      ) {
        NotificationManager.error(
          "Houve erro: As quantidades requisitadas sao superiores ao stock",
          "Erro",
    ...
    if (phoneNumber !== "" && !phoneRegex.test(phoneNumber)) {
      ...
      // Show a notification error using the notification manager
      NotificationManager.error(
        "Houve erro: A quantidade requisitada é superior ao stock",
        "Erro",
        8000
      );
      exitEditingMode(); //required to exit editing mode
    ...
    if (
      originalRequisicao.quantidade !== "" &&
      parseInt(originalRequisicao.quantidade) > 0
    ) {
  ...
  if (!materialRequisicao) {
    ...
    if (errorGetMaterial !== null) {
      // Show a notification error using the notification manager
      NotificationManager.error(
        "Houve um erro : " + errorGetMaterial.message,
        "Erro",
        8000
      );
      return (
        ...
        <>
          <main className="h-full">
            ...
            <div className="mainCard">
              ...
              {materialRequisicao.length > 0 ? (
                <span>
                  {" "}
                  <div className="border w-full border-gray-200 bg-white py-4 px-6 rounded-md us-header ">
                    <p>
                      Selecione os destinos para onde prentende distribuir o
                      Material:{" "}
                    </p>
                    {/* 
              <div className="inline-grid  grid-cols-4 destino-distritos"> */}
                  ...
                  <div className="border w-full border-gray-200 bg-white py-4 px-6 rounded-md ">
                    ...
                    <span className="alert-saldo">
                      Saldo Remanescente:{" "}
                      {materialRequisicao[0].qtd_stock - totalQuantidadeReq}
                    </span>
                    {loadingRequisicao ? (
