...
import { getAllArmazens ,getPontosFocais } from "../../middleware/GenericService";
...
import { OutlinedInput, InputLabel, Select, MenuItem, Chip, FormControl } from '@mui/material';
...
const handleRegistarPontoFocal = async () => {
 ...
 const contacto = document.getElementById("contacto").value;
  ...
  const unidade_sanitaria = selectedUsRef.current.unidade_sanitaria;
  ...
  let currentUserArea = userArea;
   const areaInfo = areasProgramaticas.filter(item => userArea.includes(item.area));
  ...
  const pontoFocal ={
    ...
    id_area: area_id
      ...
      try {
        ...
        .catch(error => {
          // handle any error state, rejected promises, etc..
          NotificationManager.error("Houve erro ao buscar a lsita dos PF:" + error.message , 'Erro: ' +error.code, 8000);
        }
        );
         ...
         return;
          ...
          if(error.response.data.message==="Ponto Focal already exists"){
           NotificationManager.error("Ponto Focal : { " + nome + " } ja existe!", 'Duplicado', 8000);
   
         } else {
           NotificationManager.error("Houve erro ao registar o PF:" + error.message , 'Erro: ' +error.code, 8000);
         }
         }
};
  ...
  <div className="px-2 mx-auto mainCard">
    ...
    <div className="px-2 mx-auto mainCard">
            ...
            <TableRow>
              ...
              <TableBody>
                 <TableRow >
                    ...
                    <TextField required id="nome" label="Nome :" variant="standard"    sx={{ width: 300 }}/>	
                    </TableCell>
                    <TableCell align = "left"> 
                    <TextField  required id="contacto" label="Contacto :" variant="standard" sx={{ width: 300 }} />	
                    </TableCell>
                    <TableCell align = "left">
                    <Autocomplete
                    id="grouped-demo"
      ...
      <TableCell></TableCell>
        <TableCell align='center' > <button  onClick={handleRegistarPontoFocal} className="py-2 px-4 border border-blue-500 bg-blue-700  rounded text-gray-200 hover:bg-blue-600 hover:border-blue-600 justify-end">
           Salvar
          </button></TableCell>
         ...
         <h1 className="text-slate-500 pb-3 text-base md:text-lg">
          ...
          <PontosFocaisTable colunas={columnNamesPontoFocal} dados={pontosFocais} />
