...
useEffect(() => {
  ...
  .catch(error => {
   ...
   NotificationManager.error('Unable to get Guias de Saida...' + error.message,'Erro getGuiasByArea', 5000);
    setHasFinishedLoading(true);
  });
  ...
  if( !listaGuias){
    return (
      ...
      <span className="us-header" ><p> Lista de Guias de Saida: </p></span>
  ...
  // if there are no requisicoes pendentes, display a message using the NotificationManager
    NotificationManager.info('Nao existem Guias de Saida...','Sem dados', 8000);
                ...
                <Box sx={{ borderBottom: 1, borderColor: 'divider' }}>
                  ...
                  >
                    {guiasAreaCount?.map((data, index) => (
                     ...
                     <Tab label=  {data.area + " (" + data.total_guias + ")"} value={data.area} />
                  ))}
                  </TabList>
                </Box>
                 {guiasAreaCount?.map((data, index) => (
                  ...
                  <TabPanel value={data.area} >
                    <GuiasPorAreaTable colunas = {columnNamesListaGuias} 
                     dados = { listaGuias.filter((guia) => guia.area === data.area) }   />
